...
const QuizDetails: React.FC = () => {
  ...
  useEffect(() => {
    //const refreshFlag = new URLSearchParams(location.search).get("refreshAttempts");
    ...
    fetchQuiz();
    fetchAttempts();
  }, [qid, dispatch, user?._id, location.search]);
  ...
  const canTakeQuiz = (): boolean => {
    ...
    const now = new Date();
    if (now < new Date(quiz.availableDate) || now > new Date(quiz.untilDate)) {
      return false;
    }
  ...
  const getStudentAttemptsInfo = (): React.ReactNode => {
    ...
    return (
      <div className="student-attempts-info">
        <p>
          <strong>Your attempts:</strong> {attemptsUsed} of {maxAttempts}
        </p>
        <p>
        ...
        {attemptsUsed > 0 && (
          <p>
            <strong>Last score:</strong> {lastScore} / {quiz.points} (
