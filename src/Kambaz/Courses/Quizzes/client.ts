...
export const createQuizAttempt = async (quizId: string, attempt: Omit<QuizAttempt, '_id'>) => {
...
export const findQuizAttemptsByQuizAndUser = async (quizId: string, userId: string, isPreview: boolean = false) => {
  const response = await axios.get(
    `${QUIZZES_API}/${quizId}/attempts?userId=${userId}&isPreview=${isPreview}`
  );
  return response.data;
};
